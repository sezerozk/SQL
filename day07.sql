...
/*	JOIN: ilişkili tablolarda ortak bir sütun baz alınarak(tipik olarak PK ve FK) bir veya daha fazla 
	tablodaki sütunların birleştirilmesini sağlar	*/


CREATE TABLE sirketler  (	--sirketin tanimlayici bilgileri
sirket_id int,  
...
CREATE TABLE siparis (	--sirketin siparis detaylari tablosu
siparis_id int,
sirket_id int,
siparis_tarihi date
);
...
INSERT INTO siparis VALUES(55, 105, ' 2022-12-21');
	...
	/*	Tablolar birbirine bagli olmasina ragmen Constraint (PK ve FK) kullanmadik.  Buna Soft İliski demistik	*/
...
--şirketler tablosundaki tüm kayıtların şirket id,şirket ismi, sipariş tarihini,sipariş idsini  listeleyiniz.    
SELECT sirketler.sirket_id,sirket_isim,siparis_id,siparis_tarihi
FROM sirketler LEFT JOIN siparis
ON sirketler.sirket_id=siparis.sirket_id
   
--siparis tablosundaki tüm kayıtların şirket id,şirket ismi, sipariş tarihini,sipariş idsini  listeleyiniz.    
...
--Ismi içinde e ve a olan devlerin tüm bilgilerini yazdiran QUERY yazin --kemal,hande gibi
SELECT *
FROM developers
