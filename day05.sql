...
WHERE isyeri='Vakko'
--NOT:14,15,16 is Concat updated?? concat vs ||(about null)--bu versiyonlarda concat olmuyor mu, arastiralim.
...
UPDATE calisanlar4
SET isim='MISSING...'
WHERE isim IS NULL	--burada isim=Null kullanamiyoruz cunku null esitlenecek bir deger degil...!!!
SELECT * FROM calisanlar4
...
SELECT *
FROM person
ORDER BY adres	--Default olarak ASC - ascending,natural,artan(default) siralar

--ASC siralama - ARTAN, Ascendind
SELECT *
FROM person
ORDER BY adres ASC--ascending,natural,artan(default)

--person tablosundaki tüm kayıtları soyisim göre (azalan) sıralayarak listeleyiniz.
SELECT *
FROM person
...
-- ayni komut icindeki gecisici isim, order by ile de kullanilabilir.
SELECT isim,soyisim,LENGTH(soyisim) AS karakter_sayisi
FROM person
...
--------- ayrı ayrı length vermek yerine concat yapsak...
