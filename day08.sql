...
-- lastname değerlerinin küçük harfe çevirerek uzunlukları ile birlikte listeleyiniz.
...
--calisanlar4 tablosunda isyeri='Vakko' olan kayıtlarda son ' Şubesi' ifadesini siliniz.
UPDATE calisanlar4
...
--NOT:içinde kayıtlar bulunan bir tabloyu güncellerken NOT NULL,PK,FK,UNIQUE,CHECK 
--kısıtlamalarını ekleyebilmek için bu sütunun değerleri ilgili kısıtlamayı sağlıyor olmalı
...
--sistemsel hata oluştu (birbirine bağlı bu işlemleri ayrı ayrı çalıştıtrınca işlem yarıda kalınca hata veriyor )
UPDATE hesaplar SET bakiye=bakiye+1000 WHERE hesap_no=5678;
--bu nedenle tek bir transaction ile birbirine bağlamalıyız
SELECT * FROM hesaplar;
-------------------------------------------------------------------
--yeni TRANSACTION Baslatma
BEGIN;
CREATE TABLE hesaplar
(
hesap_no int UNIQUE,
