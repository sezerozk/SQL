...
--PK:''(yani empty) NULL değildir, bu nedenle ustte id yi kabul etti.
...
INSERT INTO ogrenciler1 VALUES(122, 'Kerem Can', 'Fatma',75),	--coklu kayit girme
...
--id=10002 kayıt adresler tarafından ref alındığı için FK kısıtlaması silmeye izin vermez.

DELETE FROM adresler WHERE adres_id='10002';-- once ref alınan kayıt child Table'dan silinerek ilişki koparıldı..
DELETE FROM calisanlar WHERE id='10002';	--artık ref olmadığından parent table'da silmeye izin verir

--tum kayitlari silmek istersek;
DELETE FROM adresler; --önce ref alan tüm kayıtlar child table'dan silinir ve tüm ilişkiler de koparılır
DELETE FROM calisanlar;--sonra parent table'da tüm kayıtların silinmesine izin verdi.



--18-ON DELETE CASCADE:kademeli silme işlemini otomatik gerçekleştirir

CREATE TABLE talebeler( --parent
id int primary key,  
isim VARCHAR(50),
veli_isim VARCHAR(50),
yazili_notu int
...
--notlar (child) tablosundan id=123 olan kaydı silelim
DELETE FROM notlar WHERE talebe_id=123;

--talebeler (parent) tablosundan id=126 olan kaydı silelim
DELETE FROM talebeler WHERE id=126; --onDeleteCcascade ile önce ref alan kayıt sonra ilgili kayıt tablodan silindi.

DELETE FROM talebeler;	--parent
--on delete cascade öz ile tüm referanslar silindi ilişkili kayıt kalmadığı için tüm kayıtları sildi.
DELETE FROM notlar;	--child		yani ilişkili kayıtları silmiş oldu


--19-tabloyu SCHEMAdan silme:
DROP TABLE talebeler;
DROP TABLE talebeler CASCADE;
--notlar tablosunda tanımlı FK constrainti kaldırlır ardından tablo silinir.
